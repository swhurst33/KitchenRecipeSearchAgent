...
class UserContextLoader:
    ...
    async def load_user_context(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Loaded user context for {user_id}: diet={context['diet_type']}, "
    ...
    async def _get_user_settings(self, user_id: str) -> Dict[str, Any]:
        ...
        """
        try:
            response = self.supabase.table('user_settings').select('diet_type, allergies, disliked_ingredients').eq('user_id', user_id).execute()
            ...
            if response.data:
                ...
                return {
                    ...
                    'disliked_ingredients': settings.get('disliked_ingredients', []) or []
                }
            else:
    ...
    async def _get_hated_recipe_urls(self, user_id: str) -> List[str]:
        ...
        try:
            ...
            if response.data:
                urls = [recipe['source_url'] for recipe in response.data if recipe.get('source_url')]
    ...
    async def _get_saved_recipe_urls(self, user_id: str) -> List[str]:
        ...
        """
        try:
            response = self.supabase.table('saved_recipes').select('source_url').eq('user_id', user_id).execute()
    ...
    def enhance_prompt_with_context(self, original_prompt: str, context: Dict[str, Any]) -> str:
        """
...
class UserPreferences:
    ...
    def enhance_prompt_with_preferences(self, original_prompt: str, preferences: Dict[str, Any]) -> str:
        ...
        context = {
            ...
            'exclude_ingredients': preferences.get('allergens', []) + preferences.get('disliked_ingredients', [])
        }
